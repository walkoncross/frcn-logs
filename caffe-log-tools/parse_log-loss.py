...
def parse_log(path_to_log):
    ...
    #regex_train_output = re.compile('Train net output #(\d+): ([\S\_]+) = ([\.\deE+-]+)')
    #regex_test_output = re.compile('Test net output #(\d+): ([\S\_]+) = ([\.\deE+-]+)')
    regex_train_output = re.compile('Iteration (\d+), (lo[\S\_]+) = ([\.\deE+-]+)')
    #regex_test_output = re.compile('Test net output #(\d+): ([\S\_]+) = ([\.\deE+-]+)')
...
def save_csv_files(logfile_path, output_dir, train_dict_list, test_dict_list,
    ...
    train_filename = os.path.join(output_dir, log_basename + '_loss.train')
    write_csv(train_filename, train_dict_list, delimiter, verbose)

    test_filename = os.path.join(output_dir, log_basename + '_loss.test')
...
def main():
    ...
    print 'len(train_dict_list) = ', len(train_dict_list)
    print 'len(test_dict_list) = ', len(test_dict_list)
